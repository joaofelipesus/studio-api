...
RSpec.describe 'Personals::MuscularGroups', type: :request do
  ...
  describe 'GET /api/personal/muscular_groups' do
    let!(:create_muscular_groups) { 3.times { create(:muscular_group) } }

    before(:each) { get('/api/personal/muscular_groups', headers: headers(user: personal.user)) }
